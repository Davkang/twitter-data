...
# ===== to get twitter usernames =====
# twitter_data.each do |hash|
#   puts hash.keys.first
# end

# ===== its description =====
puts "\n =====# usernames & its descriptions #======"

twitter_data.each do |hash|

  if hash[hash.keys.first]["description"].nil?
    puts "#{hash.keys.first}: NA"
  else
    puts "#{hash.keys.first}: #{hash[hash.keys.first]["description"]}"
  end
end

puts " \n =====# usernames & number of followers #======"
twitter_data.each do |hash|
...
puts " \n =====# usernames and the length of their latest tweets #====="
# each username and the length of their latest tweets

twitter_data.each do |hash|
  puts "#{hash.keys.first}'s latest tweet was #{hash[hash.keys.first]["latest tweet"].length} characters long."
end

# each username followed by the total character count used in that user's last twenty tweets
puts " \n =====# total chracter count in user's last twenty tweets #====="
...
twitter_data.each do |hash|
  ...
  puts "#{hash.keys.first} used #{character_count} in their last twenty tweets."
end

# which user has the most followers?
puts "\n =====# A user with the most followeres #====="

user_names = ""
max_follower = 0

twitter_data.each do |hash|
  if max_follower < hash[hash.keys.first]["number of followers"]
    user_names = hash.keys.first
    max_follower = hash[hash.keys.first]["number of followers"]
  end
end

puts "#{user_names} has the most followers"

# which user has the most friends?
puts "\n ===== # which user max friends # ====="

user_names = ""
max_friend = 0

twitter_data.each do |hash|
  if max_friend < hash[hash.keys.first]["number of friends"]
    user_names = hash.keys.first
    max_friend = hash[hash.keys.first]["number of friends"]
  end
end

puts "#{user_names} has the most friends"

# which user has the greatest number of tweets?
puts "\n =====# A user with the greatest number of tweets #====="

user_names = ""
max_tweets = 0

twitter_data.each do |hash|
  if max_tweets < hash[hash.keys.first]["number of tweets"]
    user_names = hash.keys.first
    max_tweets = hash[hash.keys.first]["number of tweets"]
  end
end
puts "#{user_names} has the greatest number of tweets"

# which users have a description listed in twitter_data?

puts " \n ===== # users with description listed # ====="
twitter_data.each do |hash|
...
puts "\n ===== # users with location listed # ====="
twitter_data.each do |hash|

  if hash[hash.keys.first]["location"].nil?
